...
export class InventoryService implements OnModuleInit {
  ...
  async onModuleInit() { this.sha = (await this.redis.script('LOAD', this.script)) as string; }

  async holdSeat(eventId: string, seatId: string, userId: string): Promise<HoldResult> {
    const ttl = Number(process.env.SEAT_HOLD_TTL_SEC ?? 300);
    const res = (await this.redis.evalsha(
      this.sha, 2, `event:${eventId}:stock`, `event:${eventId}:seats`, seatId, userId, String(ttl),
    )) as number;
